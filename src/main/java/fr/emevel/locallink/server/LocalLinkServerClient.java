...
public class LocalLinkServerClient extends LinkSocket {
    ...
    public LocalLinkServerClient(LocalLinkServerData data, Socket socket, Runnable dataSaver) throws IOException {
        super(socket);
        this.data = data;
    ...
    public void createLink(SyncFolder syncFolder, String folder) {
        LocalLinkServerData.UserInfo userInfo = data.getOrCreateUser(uuid.toString());
        userInfo.getFolders().add(syncFolder.getUuid());
        dataSaver.run();
    ...
    private void handshake(PacketHandShake packet) {
        ...
        LocalLinkServerData.UserInfo userInfo = data.getOrCreateUser(packet.getUuid().toString());
        List<UUID> dataFolders = userInfo.getFolders();
        if (dataFolders == null || dataFolders.isEmpty()) {
            return;
        }
        for (UUID folderId : userInfo.getFolders()) {
